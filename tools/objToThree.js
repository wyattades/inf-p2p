...
const LOADERS = {
  STL: () => require('three/examples/js/loaders/STLLoader'),
  OBJ: () => require('three/examples/js/loaders/OBJLoader'),
  MTL: () => require('three/examples/js/loaders/MTLLoader'),
};
...
//       if (
//         res.statusCode === 200 &&
//         /javascript/.test(res.headers['content-type'])
//       ) {
//         let rawData = '';
...
(async () => {
  ...
  if (loader === 'OBJ') {
    ...
    if (match && match[1]) {
      ...
      materials.preload();
      obj = new THREE.OBJLoader().setMaterials(materials).parse(inputData);
    } else {
